...
try:
    ...
    for item in INFRASTRUCTURE_LIST:
        ...
        #arcpy.AddMessage(INFRA_FEATURE_LAYER_LIST)
    LOGGER.info("Starting with the Hazards processing")

    with arcpy.da.UpdateCursor(TARGET_FC, ['OBJECTID', 'INFRASTRUCTURE',
                                           'INFRA_BUFFER_DIST'],
        ...
        for row in cursor:
            ...
            LOGGER.info("Processing OID " + str(row[0]) +
                        ", with INFRASTRUCTURE grading of " + str(row[1]) +

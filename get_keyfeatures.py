...
"""
# Get the count of KeyFeatures (OSM Buildings shapefile) within a predefined
radius and assign a weight based on the count:
...
REQUIRED_FIELD = "KEYFEATURES" # Which field must we filter on and check for?
...
try:
    ...
    arcpy.MakeFeatureLayer_management(TARGET_FC, "inputHazard", FILTER_QUERY)  # SelectLayer_management -> The input cannot be the path to a feature class on disk.
    ...
    #Create feature layers out of Key Features FC
    LIST_COUNT = 0
    ...
    for item in KEYFEATURECLASS_LIST:
        ...
        #arcpy.AddMessage(listKeyFeatLayer)

    arcpy.AddMessage("Starting with the Hazards processing....")

    with arcpy.da.UpdateCursor(TARGET_FC, ['OBJECTID', 'KeyFeatures',
                                           'KEYFEATURES_BUFFER_DIST'],
        ...
        for row in cursor:
            ...
            arcpy.AddMessage("Processing OID " + str(row[0]) +
                             ", with KEYFEATURES grading of "+ str(row[1]) +
                             ". Feature " + str(COUNTER) + " of " +
                             str(RECORD_COUNT) + " (" + str(pctDone) + " %)")
            ...
            for fc in KEYFEATURELAYER_LIST:
                ...
                outputKeyFeat = arcpy.SelectLayerByLocation_management(fc,
                ...
                for row2 in outputKeyFeat:
                    ...
                    #                                        skip_nulls=True)
                    # Should we do the skip_nulls?
                    #arcpy.AddMessage("Total Infrastructure in radius: " + str(arr["OID@"].sum()))
                    #totPop = arr["grid_code"].sum()

                #arcpy.AddMessage("ROW countKeyFeat is: " + str(countKeyFeat))

            # Update the row with the population sum
            #arcpy.AddMessage("countKeyFeat is: "+ str(countKeyFeat))
            # Cast to integer to ensure we deal with integer values
            countKeyFeat = int(countKeyFeat)
    ...
    STOP_TIME = time.time()
    arcpy.AddMessage("\nDone with Key Features Proximity Analysis")

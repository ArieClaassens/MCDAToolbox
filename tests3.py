...
with arcpy.da.UpdateCursor(featureclass, ["SHAPE@", "OBJECTID", "SW", "S", "SE", "W", "CENTER", "E", "NW", "N", "NE"]) as cursor:
     for row in cursor:
        print("########################################################################################################################################################")
        print(" ")
        extent = row[0].extent
        print("DHA {0}:".format(row[1]))
        print("SW: {0}. S: {1}. SE: {2}. W: {3}. CENTER: {4}. E: {5}. NW: {6}. N: {7}. NE: {8}".format(row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10]))
        print("XMin: {0}, YMin: {1}, XMax: {0}, YMax: {1}".format(extent.XMin, extent.YMin, extent.XMax, extent.YMax))

...
# Breaks need to be redefined, currently all sites are falling in the high range.
...
REQUIRED_FIELDS = ['LANDCOVER', 'ASPECT',
                   'INFRASTRUCTURE', 'KEYFEATURES',
                   'ACCIDENTS', 'POI', 'RIVERS', 'SLOPE',
                   'POPULATION', 'SCORE', 'RANKING',
                   'LANDCOVERWEIGHT', 'ASPECTWEIGHT',
                   'INFRASTRUCTUREWEIGHT',
                   'KEYFEATURESWEIGHT', 'ACCIDENTSWEIGHT',
                   'POIWEIGHT', 'RIVERSWEIGHT',
                   'SLOPEWEIGHT', 'POPULATIONWEIGHT',
                   'WEIGHTEDSCORE']
...
try:
    ...
    if check_weights_same(weightlist):
        LOGGER.error("You need to assign at least TWO unique weights to \
                      the different factors. You cannot use the same weight \
    ...
    # Check if the target feature class has the required attribute fields.
    ...
    arcpy.MakeTableView_management(TARGET_FC, "tableViewHazards")
    RECORD_COUNT = int(arcpy.GetCount_management("tableViewHazards").getOutput(0))
    ...
    with arcpy.da.UpdateCursor(TARGET_FC, ['OBJECTID', 'LANDCOVER', 'ASPECT',
                                           'INFRASTRUCTURE', 'KEYFEATURES',
                                           'ACCIDENTS', 'POI', 'RIVERS', 'SLOPE',
                                           'POPULATION', 'SCORE', 'RANKING',
                                           'LANDCOVERWEIGHT', 'ASPECTWEIGHT',
